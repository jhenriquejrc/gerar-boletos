...
module.exports = class Boletos {
  ...
  gerarBoleto() {
    ...
    const { datas, valor, especieDocumento, numeroDocumento, parcela } = this.boleto;
  ...
  pdfBase64() {
    return new Promise((resolve) => new Boleto.Gerador(this.boletoInfo).gerarPDF({
      creditos: 'Joao Henrique',
      base64: true,
    }).then(({base64, linhaDigitavel}) => resolve({ boleto: this.boleto,  base64, linhaDigitavel })));
  }
};